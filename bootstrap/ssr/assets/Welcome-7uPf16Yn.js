...
const _sfc_main = {
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="h-screen overflow-y-auto snap-y snap-mandatory scroll-smooth no-scrollbar bg-background text-brand-ink font-sans selection:bg-brand-orange selection:text-white relative"><nav class="border-b border-brand-hairline bg-background/95 backdrop-blur-sm fixed top-0 left-0 right-0 z-50 px-6 flex items-center justify-between w-full pt-[calc(0.25rem+env(safe-area-inset-top,0px))] pb-2 min-h-[calc(4rem+env(safe-area-inset-top,0px))]"><div class="flex items-center gap-8">`);
      ...
      _push(`</div></nav><section class="h-screen w-full snap-start snap-always shrink-0 flex flex-col justify-between relative overflow-hidden border-b border-brand-hairline bg-background pt-[calc(4.5rem+env(safe-area-inset-top,0px))]"><div class="container mx-auto px-6 flex-1 flex flex-col justify-center items-center text-center relative z-10 py-12 max-w-4xl"><div class="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-brand-hairline bg-brand-surface-strong text-[11px] font-semibold text-brand-ink mb-6">`);
