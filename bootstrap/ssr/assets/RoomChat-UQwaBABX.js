    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(messages.value, (msg) => {
        ...
        _push(`<div class="${ssrRenderClass([
          ...
          msg.user_id === __props.user.id ? "bg-zinc-900 text-zinc-50 rounded-tr-none" : "bg-zinc-100 text-zinc-800 rounded-tl-none border border-zinc-200"
      ...
      _push(`<!--]--></div><div class="p-2 border-t border-zinc-100 bg-white flex-shrink-0"><form class="flex gap-2 items-center bg-zinc-50 p-0.5 pl-2.5 rounded-full border border-zinc-200 focus-within:border-zinc-400 focus-within:ring-1 focus-within:ring-zinc-400/20 transition-all"><input${ssrRenderAttr("value", newMessage.value)} type="text" placeholder="اكتب رسالة..." class="flex-1 bg-transparent border-none focus:ring-0 text-xs text-zinc-800 placeholder-zinc-400 py-1.5"${ssrIncludeBooleanAttr(isSending.value) ? " disabled" : ""}><button type="submit"${ssrIncludeBooleanAttr(!newMessage.value.trim() || isSending.value) ? " disabled" : ""} class="h-8 w-8 flex items-center justify-center bg-zinc-900 hover:bg-zinc-800 disabled:bg-zinc-100 disabled:text-zinc-400 text-white rounded-full transition-colors flex-shrink-0"><svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 transform rotate-180" viewBox="0 0 20 20" fill="currentColor"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z"></path></svg></button></form></div></div>`);
    };
  }
};
