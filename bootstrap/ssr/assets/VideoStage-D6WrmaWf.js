  ...
  setup(__props) {
    ...
    const broadcastVideoSync = () => {
      ...
      const state = {
        ...
        // Enforce absolute ground-truth playback state
        current_time: player.currentTime,
        volume: player.volume,
        // Enforce absolute ground-truth volume state
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({
        class: "relative w-full h-full bg-black rounded-2xl overflow-hidden shadow-lg group flex items-center justify-center",
      ...
      if (isLeader.value) {
        _push(`<div class="absolute top-4 left-4 z-30 bg-white/90 backdrop-blur-sm p-2 rounded-xl shadow-lg flex items-center gap-3 transition-opacity opacity-0 group-hover:opacity-100"><button class="${ssrRenderClass(["p-2 rounded-lg transition-colors", isDrawingMode.value ? "bg-indigo-600 text-white" : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200"])}" title="تفعيل الرسم"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></button>`);
        if (isDrawingMode.value) {
          _push(`<div class="flex items-center gap-2 border-r border-zinc-200 pr-3 mr-1"><input type="color"${ssrRenderAttr("value", brushColor.value)} class="w-8 h-8 rounded cursor-pointer border-0 p-0"><input type="range"${ssrRenderAttr("value", brushSize.value)} min="1" max="20" class="w-24 accent-indigo-600"><button class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="مسح الكل"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
      ...
      if (!isLeader.value) {
        _push(`<div class="absolute top-4 right-4 z-30 bg-black/60 backdrop-blur-sm text-white px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span> يتم العرض بواسطة القائد </div>`);
      } else {
        _push(`<!---->`);
      }
      if (isLeader.value) {
        _push(`<div class="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 bg-indigo-950/85 backdrop-blur-md px-4 py-2 rounded-2xl shadow-xl flex items-center gap-4 transition-all opacity-0 group-hover:opacity-100 border border-indigo-500/25 text-white select-none"><div class="flex items-center gap-2 border-l border-indigo-800/80 pl-3"><span class="relative flex h-2 w-2"><span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span></span><span class="text-[10px] font-medium text-emerald-400 tracking-wider">بث مباشر للطلاب</span></div><button class="p-2 rounded-xl bg-indigo-900/50 hover:bg-indigo-800 text-indigo-200 hover:text-white transition-all hover:scale-105 active:scale-95" title="إرجاع 10 ثواني"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12.066 11.2a1 1 0 000 1.6l5.334 4A1 1 0 0019 16V8a1 1 0 00-1.6-.8l-5.334 4z"></path><path stroke-linecap="round" stroke-linejoin="round" d="M4.066 11.2a1 1 0 000 1.6l5.334 4A1 1 0 0011 16V8a1 1 0 00-1.6-.8l-5.334 4z"></path></svg></button><button class="p-3 rounded-full bg-amber-500 hover:bg-amber-600 text-white font-bold transition-all shadow-md hover:scale-105 active:scale-95 flex items-center justify-center"${ssrRenderAttr("title", isPlaying.value ? "إيقاف مؤقت" : "تشغيل")}>`);
        if (isPlaying.value) {
        ...
        _push(`</button><button class="p-2 rounded-xl bg-red-600 hover:bg-red-700 text-white transition-all hover:scale-105 active:scale-95" title="إيقاف كامل"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M4.5 7.5a3 3 0 0 1 3-3h9a3 3 0 0 1 3 3v9a3 3 0 0 1-3 3h-9a3 3 0 0 1-3-3v-9Z" clip-rule="evenodd"></path></svg></button><button class="p-2 rounded-xl bg-indigo-900/50 hover:bg-indigo-800 text-indigo-200 hover:text-white transition-all hover:scale-105 active:scale-95" title="تقديم 10 ثواني"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11.934 12.8a1 1 0 000-1.6L6.6 7.2A1 1 0 005 8v8a1 1 0 001.6.8l5.334-4z"></path><path stroke-linecap="round" stroke-linejoin="round" d="M19.934 12.8a1 1 0 000-1.6l-5.334-4A1 1 0 0013 8v8a1 1 0 001.6.8l5.334-4z"></path></svg></button></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
