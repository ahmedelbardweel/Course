...
import _sfc_main$3 from "./RoomChat-UQwaBABX.js";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_sfc_main$1, null, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          var _a, _b;
          if (_push2) {
            _push2(`<div class="h-[calc(100vh-80px)] xl:h-[calc(100vh-64px)] overflow-hidden flex flex-col p-2 sm:p-4 max-w-[1600px] mx-auto min-h-0"${_scopeId}><div class="bg-white border border-zinc-200 px-4 py-2.5 rounded-t-xl flex items-center justify-between shadow-sm text-zinc-900 relative flex-shrink-0"${_scopeId}><div${_scopeId}><h1 class="text-sm sm:text-base font-bold flex items-center gap-1.5 text-zinc-900"${_scopeId}><span class="w-2 h-2 bg-red-500 rounded-full animate-pulse"${_scopeId}></span> ${ssrInterpolate(__props.room.course.title)}</h1><p class="text-zinc-500 text-[10px] sm:text-xs mt-0.5 flex items-center gap-1.5"${_scopeId}> القائد الحالي: ${ssrInterpolate(((_a = __props.room.leader) == null ? void 0 : _a.name) || "غير محدد")} `);
            if (!isLeaderPresent.value && !isLeader.value) {
              _push2(`<span class="bg-red-100 text-red-700 px-1.5 py-0.5 rounded text-[9px] border border-red-200"${_scopeId}> غير متواجد </span>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</p></div>`);
            if (!isLeaderPresent.value && !isLeader.value) {
              _push2(`<div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2"${_scopeId}><button class="bg-zinc-900 hover:bg-zinc-800 text-white px-3 py-1 rounded-lg text-[10px] sm:text-xs font-bold shadow transition-all flex items-center gap-1 animate-bounce"${_scopeId}> المطالبة بالقيادة </button></div>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`<div${_scopeId}>`);
            if (isLeader.value) {
              _push2(`<span class="bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold border border-emerald-200 flex items-center gap-1.5"${_scopeId}><span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"${_scopeId}></span> أنت تبث شاشتك </span>`);
            } else {
              _push2(`<span class="bg-zinc-50 text-zinc-700 px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold border border-zinc-200 flex items-center gap-1.5"${_scopeId}><span class="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"${_scopeId}></span> يتم العرض بواسطة القائد </span>`);
            }
                ...
                createVNode("div", { class: "bg-white border border-zinc-200 px-4 py-2.5 rounded-t-xl flex items-center justify-between shadow-sm text-zinc-900 relative flex-shrink-0" }, [
                  createVNode("div", null, [
                    createVNode("h1", { class: "text-sm sm:text-base font-bold flex items-center gap-1.5 text-zinc-900" }, [
                      createVNode("span", { class: "w-2 h-2 bg-red-500 rounded-full animate-pulse" }),
                    ...
                    createVNode("p", { class: "text-zinc-500 text-[10px] sm:text-xs mt-0.5 flex items-center gap-1.5" }, [
                      ...
                      !isLeaderPresent.value && !isLeader.value ? (openBlock(), createBlock("span", {
                        key: 0,
                        class: "bg-red-100 text-red-700 px-1.5 py-0.5 rounded text-[9px] border border-red-200"
                  ...
                  !isLeaderPresent.value && !isLeader.value ? (openBlock(), createBlock("div", {
                    ...
                    createVNode("button", {
                      ...
                      class: "bg-zinc-900 hover:bg-zinc-800 text-white px-3 py-1 rounded-lg text-[10px] sm:text-xs font-bold shadow transition-all flex items-center gap-1 animate-bounce"
                    }, " المطالبة بالقيادة ")
                  ])) : createCommentVNode("", true),
                  createVNode("div", null, [
                    isLeader.value ? (openBlock(), createBlock("span", {
                      key: 0,
                      class: "bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold border border-emerald-200 flex items-center gap-1.5"
                    }, [
                      createVNode("span", { class: "w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" }),
                      createTextVNode(" أنت تبث شاشتك ")
                    ])) : (openBlock(), createBlock("span", {
                      key: 1,
                      class: "bg-zinc-50 text-zinc-700 px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold border border-zinc-200 flex items-center gap-1.5"
                    }, [
                      createVNode("span", { class: "w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" }),
